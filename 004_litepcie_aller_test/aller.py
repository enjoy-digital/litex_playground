...
class PCIeSoC(SoCCore):
    def __init__(self, platform, pcie="pcie_x4", **kwargs):
        ...
        # PCIe -------------------------------------------------------------------------------------
        # PHY
        self.submodules.pcie_phy = S7PCIEPHY(platform, platform.request(pcie),
            data_width = 64 if pcie == "pcie_x1" else 128,
            bar0_size  = 0x20000)
        ...
        self.submodules.pcie_dma0 = LitePCIeDMA(self.pcie_phy, self.pcie_endpoint,
            with_buffering = True, buffering_depth=8192,
            with_loopback  = True)
    ...
    def generate_software_headers(self):
        ...
        tools.write_to_file("software/kernel/csr.h", csr_header)
        soc_header = get_soc_header(self.constants, with_access_functions=False)
        tools.write_to_file("software/kernel/soc.h", soc_header)
        mem_header = get_mem_header(self.mem_regions)
        tools.write_to_file("software/kernel/mem.h", mem_header)
